			...
			while ($data = mysqli_fetch_assoc($readall)) {
				foreach($data as $field => $value) {
					if ($data[$field] == "") {
						...
						$img_file = "../assets/images/services/".$image;
						
						if(file_exists($img_file))
						{
						    $whole_url = SITE_URL."assets/images/services/".$image;
						} else {
						    $whole_url = SITE_URL."assets/images/services/default_service.png";
						}
...
elseif($_POST['action'] && $_POST["action"] == "social_login"){
    	verifyRequiredParams(array("api_key", "social_id",'social_type'));
	if (isset($_POST["api_key"]) && $_POST["api_key"] == $objsettings->get_option("ct_api_key")) {
	    
		$user->social_id = trim(strip_tags(mysqli_real_escape_string($conn, $_POST["social_id"])));
		
		$user->social_type = trim(strip_tags(mysqli_real_escape_string($conn, $_POST["social_type"])));

		$existing_login = $user->check_social_login_process();
		...
		if (mysqli_num_rows($existing_login) == 0) {
			...
			$user->user_pwd = isset($_POST['password']) ? md5($_POST["password"]) :'';
			$user->first_name = isset($_POST['first_name']) ? ucwords($_POST["first_name"]):'';
			$user->last_name = isset($_POST['last_name']) ? ucwords($_POST["last_name"]):'';
			...
			if ($add_user) {
			    $new_user = $user->check_social_login_process();
			    
			    $data = mysqli_fetch_assoc($new_user);
    			if (isset($data["usertype"])) {
    				$res = unserialize($data["usertype"]);
    				$data["usertype"] = $res[0];
    				$data["fullname"] = $data["first_name"]." ".$data["last_name"];
